...
def process_gripper_pointcloud(
    ...
    pcd1_ee = pcd1.transformed(T_cam_ee1, squeeze=True)
    pcd2_ee = pcd2.transformed(T_cam_ee2, squeeze=True)
...
def main():
    ...
    parser.add_argument("--ply1",  default='/home/hkcrc/diffusion_edfs/diffusion_edf/edf_interface/pcd_test/A_1762928279832/cloud.ply')
    parser.add_argument("--ply2",  default='/home/hkcrc/diffusion_edfs/diffusion_edf/edf_interface/pcd_test/A_1762928349711/cloud.ply')
    parser.add_argument("--cam2base", default="/home/hkcrc/diffusion_edfs/diffusion_edf/tools/configs/scene_cam.json")
    parser.add_argument("--bbox", type=float, nargs=6, default=[-0.3, 0.3, -0.3, 0.3, -0.5, 0.5])
    ...
    # 获取当前末端位姿pcd2
    pos_ee2, quat_ee2 = robot.get_current_pose()
    T_base_ee2 = SE3(torch.tensor([*quat_ee2, *pos_ee2], dtype=torch.float32))
    T_cam_ee2 = SE3.multiply(T_base_ee2.inv(), T_cam_base)
    

    T_ee_rotate = SE3(torch.tensor([0., 0., 0., 1., 0., 0., 0.], dtype=torch.float32))  # 绕Z轴180°
    T_base_ee1 = SE3.multiply(T_base_ee2, T_ee_rotate)
